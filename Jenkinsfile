    ...
    // -------------------------------------------------------------------------
 	...
 	withEnv(["HOME=${env.WORKSPACE}"]) {	
		...
		stage('Authorize to Salesforce') {
			...
			rc = command "\"${toolbelt}\" force:auth:logout --targetusername ${HUB_ORG} -p & "\"${toolbelt}\" force:auth:jwt:grant --clientid ${CONNECTED_APP_CONSUMER_KEY} --username ${HUB_ORG} --jwtkeyfile \"${jwt_key_file}\" --instanceurl ${SFDC_HOST} --setdefaultdevhubusername"
